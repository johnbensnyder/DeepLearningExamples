...
from awsdet.core import preprocessor
...
def flatten_masks(masks):
    '''
    For a given set of masks, flatten into a single layer with non-zero
    values signifying the corresponding mask layer
    (only works with non-overlapping masks)
    '''
    mask_range = tf.range(1, tf.shape(masks)[0] + 1, dtype=tf.float32)
    mask_range = tf.expand_dims(tf.expand_dims(mask_range, [-1]), [-1])
    flat_masks = tf.reduce_max(masks * mask_range, axis=0)
    flat_masks = tf.expand_dims(flat_masks, [0])
    return flat_masks

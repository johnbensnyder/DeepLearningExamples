...
from awsdet.core import anchors, preprocess_ops
        ...
        with tf.name_scope('parser'):
            ...
            if mode == tf.estimator.ModeKeys.PREDICT:
                ...
                if use_instance_mask:
                    if params['flatten_masks']:
                        labels['flat_gt_masks'] = tf.expand_dims(tf.squeeze(tf.cast(instance_masks, tf.int32)), [-1])
                        mask_count = tf.shape(classes)[0]
                        if mask_count==0:
                            labels['cropped_gt_masks'] = -tf.ones([MAX_NUM_INSTANCES, 
                                                                   params['gt_mask_size'] + 4, 
                                                                   params['gt_mask_size'] + 4])
                        else:
                            bool_masks = tf.TensorArray(tf.float32, size=mask_count)
                            for i in tf.range(mask_count):
                                bool_masks = bool_masks.write(i, tf.cast(labels['flat_gt_masks']==(i+1), tf.float32))
                            instance_masks = bool_masks.stack()
...
def process_boxes_classes_indices_for_training(data, skip_crowd_during_training, use_category, use_instance_mask, flatten_masks=False):
    boxes = data['groundtruth_boxes']
    ...
    if skip_crowd_during_training:
        ...
        if use_instance_mask:
            ...
            if flatten_masks:
                instance_masks = preprocess_ops.flatten_masks(instance_masks)

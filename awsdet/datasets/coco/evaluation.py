...
def evaluate_results(results, annotations_file, use_ext=True, async_eval=True, pbar=False):
    if MPI_rank()==0:
        print("processing eval")
    worker_predictions = dict()
    if pbar and MPI_rank()==0:
        results = tqdm(results, file=sys.stdout,
                       bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')
    for a_result in results:
        processed_result = process_prediction_for_eval(a_result)
    ...
    converted_predictions = coco.load_predictions(_preds, include_mask=True, is_image_mask=False, log=pbar)
    worker_source_ids = _preds['source_id']
    ...
    if distributed_utils.MPI_rank() == 0:
        ...
        for i, s in enumerate(source_ids_list):
            source_ids.extend(s)
        # print("number of images: {}".format(len(list(set(source_ids)))))
        if use_ext:
            args = [all_predictions, annotations_file]
            fast_eval(*args)
        else:
            args = [all_predictions, source_ids, True, annotations_file]
            compute_coco_eval_metric_n(*args)

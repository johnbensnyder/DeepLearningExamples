...
IMAGES=118287
GLOBAL_BATCH_SIZE=$((BATCH_SIZE * HOST_COUNT * GPU_COUNT / 2))
...
mpirun --allow-run-as-root --mca plm_rsh_no_tree_spawn 1 \
    ...
    -x PATH \
    --oversubscribe \
    -x SMP_LOG_LEVEL=info \
    python ${BASEDIR}/../mask_rcnn_main.py \
        --mode="train_and_eval" \
	      --loop_mode="tape" \
        --checkpoint="/mask-rcnn-data/weights/resnet/resnet-nhwc-2018-02-07/model.ckpt-112603" \
        --eval_samples=5000 \
        --loop_mode="tape" \
	      --box_loss_type="giou" \
        --log_interval=100 \
        ...
        --dist_eval \
	      --first_eval=1 \
        --training_file_pattern="/mask-rcnn-data/train*.tfrecord" \
        --validation_file_pattern="/mask-rcnn-data/val*.tfrecord" \
        --val_json_file="/mask-rcnn-data/annotations/instances_val2017.json" \
        --amp \
        --use_batched_nms \
        --xla \
        --tf2 \
